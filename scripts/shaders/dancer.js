...
void mainImage(out vec4 fragColor, in vec2 fragCoord, float alpha)
{
 ...
 fragColor = vec4(vec3(colStroke * mask * iColor*2.), 1.);
...
void main() {
  ...
  mat4 rotation = mat4(
   vec4(cos(newRot), -sin(newRot), 0.0, 0.0),
   vec4(sin(newRot), cos(newRot), 0.0, 0.0),
  ...
  // Передаем позицию в растеризатор
  //gl_Position = pos;
