...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
  ...
  // Square RightDown
  float sinRDD = sin(iTime*15.)*1.1+1.1;
  vec3 o1RDD = vec3(0.0, 0.0, 1.2);
  vec3 o2RDD = vec3(0.0, 0.8, 1.2);
  vec3 omix1RDD = mix(o1RDD, o2RDD, sinRDD);
  vec3 omix2RDD = mix(o2RDD, o1RDD, sinRDD);
  vec4 colRDD = vec4(vec3(rightDownD*iColor), 1.);
  vec4 colRDU = vec4(vec3(rightDownU*iColor), 1.);
  ...
  uvRU.x -= .25;
  uvRU.y -= .25;
  uvRU = vec2(length(uvRU) * (sin(5.)+sin(iTime*5.)*2.+10.), sin(atan(uvRU.x, uvRU.y) * floor(mod(0.2, 1.5))));
  uvRU += sin(uvRU.yx * (sin(50. * 2.93) * 5.) + 1. * 4.6);
  vec3 colA = vec3(1.);
  colA *= .5 + .5 * cos(uvRU.y + 2.);
  colA = vec3(colA.r, colA.r, colA.r);
  vec3 colB = vec3(1.);
  colB *= .5 + .5 * cos(uvRU.y + iSinusRU);
  colB = vec3(0., colA.r, colB.r);
  vec3 col = mix(colA, colA, 1.0);
  col += fract(col);
  vec4 colRadRU = vec4(0.);
  colRadRU = vec4((col.rgb*iColor) * rightUp, 1.0);
  ...
  uvLU.x += 0.25;
  uvLU.y -= .25;
  uvLU = vec2(length(uvLU) * (sin(5.) + sin(iTime * 5.) * 5. + 8.), sin(atan(uvRU.x, uvRU.y) * floor(mod(0.2, 1.5))));
  uvLU += sin(uvLU.yx * (sin(50.) * 50.+15.) + 1. * 4.6);
  vec3 colRUA = vec3(1.);
  colRUA *= .5 + .5 * cos(uvLU.y + 2.);
  colRUA = vec3(colRUA.r, colRUA.r, colRUA.r);
  vec3 colRUB = vec3(1.);
  colRUB *= .5 + .5 * cos(uvLU.y + iSinusRU);
  colRUB = vec3(colRUA.r, 0., colRUB.r);
  vec3 colRU = mix(colRUA, colRUA, sin(iTime * 10.));
  colRU += fract(colRU);  
  colRadLU = vec4(colRU.rgb * leftUp * iColor, 1.0);  
  ...
  vec4 colRadLD = vec4(0., rxLD, rzLD, 1.0);
  ...
  float mask = sin(dLD * 5.0);
  vec3 ornament = vec3(mask);
  colRadLD = vec4(iColor * leftDown*ornament*1.6, 1.0);
