...
void mainImage(out vec4 fragColor, in vec2 fragCoord, float alpha)
{
 ...
 vec3 color = 20.5 + 30.5 * cos(iTime + st.xyx + vec3(0, 2, 4));
 float d = 0.0;
 st = st * 1. - .5;
 float sin_factor = sin(iTime / 5.);
 float cos_factor = cos(iTime / 5.);
 ///rand tut
 d = length(abs(sin(abs(st * .5) + iTime * .2)) * (sin(abs(cos(st.x) * sin(st * 5.)) * .8) / 2.));
 ...
 redSquare = vec4(1.0, 1.0, 1.0, 1.0);
    } else {
...
// fragColor = vec4(vec3(mask*palitra), 1.0);
 ...
 fragColor = vec4(vec3(colStroke * mask *2.*redSquare.rgb), 1.);
// fragColor = vec4(10.);
...
void main() {
  ...
  mat4 rotation = mat4(
   ...
   vec4(0.0, cos(newRot), sin(newRot), 0.0),
   vec4(0.0, -sin(newRot), cos(newRot), 0.0),
